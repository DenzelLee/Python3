...
def dispatch(request):
    ...
    elif action == 'list_lesson_checkins':
        return list_lesson_checkins(request)
    else:
...
def list_lesson_checkins(request):

    lessonid = request.params['lessonid']

    qs = StudentCheckin.objects.all().filter(lesson_id=lessonid)

    qs = qs.values('id', 'student__last_name', 'student__id') \
        .order_by('id')


    return JsonResponse({'retcode': 0, 'retlist': list(qs)})