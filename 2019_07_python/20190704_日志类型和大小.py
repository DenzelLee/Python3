...
'''
Author:lihao
Date&Time:2019-07-04 and 20:54
FileName:
'''
...
log =open(r"E:\\我的课堂\Homework\\20190704切片日志\\2019-07-04log.txt")
typeList = []
def cutLog(types,sizes):
    for num in typeList:
        if num[0] == types:
            num[1] =num[1]+sizes
            return

    typeList.append([types,sizes])#列表增加元素，可以添加列表
    return
for i in log.readlines():
    newLine =str(i.split(".")[1]).split("\t")
    type = newLine[0]
    size = newLine[1]
    # print(typeList)
    cutLog(type,int(size))
print(typeList)
